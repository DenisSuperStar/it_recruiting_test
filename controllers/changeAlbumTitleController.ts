...
class ChangeAlbumClassController {
    private readonly path: string = '/change-album-title/:albumid/:new_album_name';
    ...
    private async changeAlbumTitle(req: express.Request, res: express.Response): Promise<void> {
        const { albumid, new_album_name } = req.params;
        const { jwtToken } = req;

        if (jwtToken) {
            await Album.updateMany({ id: { $eq: albumid }}, { title: new_album_name });
        } else {
            res.json({
